...
if(nrow(multiHitFrags) > 0){
  ...
  # save.image(file = '~/dev_20220904.RData')
}
...
# leader sequence. It would be better search for the ...
...
f <- bind_rows(lapply(o, function(x){
  ...
  # Tally reads for each random id / fragStart / fragEnd
  x2 <- bind_rows(lapply(split(x, paste(x$randomLinkerSeq.adriftReads, x$fragStart, x$fragEnd)), function(y){
          o <- y[1,]
          o$reads <- n_distinct(y$readID) + sum(y$n)
          o$readIDs <- as.list(list(y$readID))
          o
