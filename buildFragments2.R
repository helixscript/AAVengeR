...
r <- data.table(readID = names(randomIDs), randomLinkerSeq = as.character(randomIDs))
...
readID2reference <- distinct(tibble(readID = anchorReadAlignments$readID, refGenome = anchorReadAlignments$refGenome.id))
