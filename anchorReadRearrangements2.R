...
r <- bind_rows(lapply(split(reads, paste(reads$vectorFastaFile, reads$sample)), function(x){
       ...
       system(paste0('blastn -dust no -soft_masking false -word_size 5 -evalue 50 -outfmt 6 -query repReads.fasta -db ',
       ...
       if(file.info('repReads.blast')$size == 0){
         ...
         b <- dplyr::filter(b, pident >= opt$anchorReadRearrangements_minAlignmentPercentID, alignmentLength >= opt$anchorReadRearrangements_mapSiteLeaderSequences_minAlignmentLength)
         ...
         readr::write_tsv(r, file.path(opt$outputDir, opt$anchorReadRearrangements_outputDir, 'repLeaderSeqStructs', paste0(x$sample[1], '.tsv')))
         altStructBreaks <- sum(stringr::str_count(r$rearrangement, ';'))
       }
       
       tibble(sample                   = x$sample[1], 
              totalReads               = ppNum(n_distinct(allReadIDs)), 
              totalNTs                 = ppNum(allReadNTs),
              expectedStructReads      = ppNum(n_distinct(expectedSeqReadsIDs)),
              expectedStructNTs        = ppNum(expectedSeqReadsNTs),
              altStructs               = ppNum(n_distinct(o$rep)), 
              altStructReads           = ppNum(n_distinct(unexpectedSeqReadsIDs)),
              altStructNTs             = ppNum(unexpectedSeqReadsNTs),
              altStructBreaks          = ppNum(altStructBreaks),
              percentAltStructReads    = sprintf("%.2f%%", (n_distinct(unexpectedSeqReadsIDs)/n_distinct(allReadIDs))*100),
              percentAltStructNTs      = sprintf("%.2f%%", (unexpectedSeqReadsNTs/allReadNTs)*100),
              percentAltStructNTs_1kb  = sprintf("%.2f%%", (unexpectedSeqReadsNTs/(allReadNTs/1000))*100),
              percentAltStructNTs_5kb  = sprintf("%.2f%%", (unexpectedSeqReadsNTs/(allReadNTs/5000))*100))
}))
