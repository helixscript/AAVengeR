...
# if(opt$alignReads_quickAlignmentFilter){
#   reads <- bind_rows(lapply(split(reads, reads$refGenome), function(x){
# 
#     write(c(paste(now(), paste0('   Starting quick anchorRead alignment filter for ', x$refGenome[1]))), file = file.path(opt$outputDir, opt$alignReads_outputDir, 'log'), append = TRUE)
#     f <- tmpFile()
#   
#     o <- DNAStringSet(unique(x$anchorReadSeq))
#     names(o) <- paste0('s', 1:length(o))
#     writeXStringSet(o, file = file.path(opt$outputDir, opt$alignReads_outputDir, 'tmp', f))
#     
#     system(paste('bwa-mem2 mem -t', opt$alignReads_CPUs, '-k', opt$alignReads_quickAlignmentFilterSeedLength, '-o',
#                   file.path(opt$outputDir, opt$alignReads_outputDir, 'tmp', paste0(f, '.sam')), 
#                   file.path(opt$softwareDir, 'data', 'blatDBs', x$refGenome[1]),
#                   file.path(opt$outputDir, opt$alignReads_outputDir, 'tmp', f)))
#     
#     i <- stringr::str_extract(readLines(file.path(opt$outputDir, opt$alignReads_outputDir, 'tmp', paste0(f, '.sam'))), '^s\\d+\\t\\d+')
#     i <- i[grepl('\\t4$', i)]
#     x$remove <- x$anchorReadSeq %in% as.character(o[names(o) %in% sub('\\t4', '', i)])
#   
#     write(c(paste(now(), paste0('   ', sprintf("%.2f%%", (sum(x$remove)/nrow(x)*100)), ' reads removed'))), file = file.path(opt$outputDir, opt$alignReads_outputDir, 'log'), append = TRUE)
#     x <- filter(x, remove == 'FALSE')
#     
#     if(nrow(x) == 0){
#       write(c(paste(lubridate::now(), 'Error - no reads remaining after quick anchor read alignment filter.')), file = file.path(opt$outputDir, opt$alignReads_outputDir, 'log'), append = TRUE)
#       if(opt$core_createFauxFragDoneFiles) core_createFauxFragDoneFiles()
#       q(save = 'no', status = 1, runLast = FALSE)
#     }
#     
#     
#     invisible(file.remove(list.files(file.path(opt$outputDir, opt$alignReads_outputDir, 'tmp'), full.names = TRUE)))
#     
#     write(c(paste(now(), paste0('   Starting quick adrift alignment filter for ', x$refGenome[1]))), file = file.path(opt$outputDir, opt$alignReads_outputDir, 'log'), append = TRUE)
#     
#     o <- DNAStringSet(unique(x$adriftReadSeq))
#     names(o) <- paste0('s', 1:length(o))
#     writeXStringSet(o, file = file.path(opt$outputDir, opt$alignReads_outputDir, 'tmp', f))
#     system(paste('bwa-mem2 mem -t', opt$alignReads_CPUs, '-k', opt$alignReads_quickAlignmentFilterSeedLength, '-o',
#                  file.path(opt$outputDir, opt$alignReads_outputDir, 'tmp', paste0(f, '.sam')), 
...
#     i <- stringr::str_extract(readLines(file.path(opt$outputDir, opt$alignReads_outputDir, 'tmp', paste0(f, '.sam'))), '^s\\d+\\t\\d+')
#     i <- i[grepl('\\t4$', i)]
#     x$remove <- x$adriftReadSeq %in% as.character(o[names(o) %in% sub('\\t4', '', i)])
#     
#     write(c(paste(now(), paste0('   ', sprintf("%.2f%%", (sum(x$remove)/nrow(x)*100)), ' reads removed'))), file = file.path(opt$outputDir, opt$alignReads_outputDir, 'log'), append = TRUE)
#     x <- filter(x, remove == 'FALSE')
#     
#     if(nrow(x) == 0){
#       write(c(paste(lubridate::now(), 'Error - no reads remaining after quick adrift read alignment filter.')), file = file.path(opt$outputDir, opt$alignReads_outputDir, 'log'), append = TRUE)
#       if(opt$core_createFauxFragDoneFiles) core_createFauxFragDoneFiles()
#       q(save = 'no', status = 1, runLast = FALSE)
#     }
#     
...
#   
#   if(nrow(reads) == 0){
#     write(c(paste(lubridate::now(), 'Error - no reads remaining after quick alignment filter.')), file = file.path(opt$outputDir, opt$alignReads_outputDir, 'log'), append = TRUE)
#     if(opt$core_createFauxFragDoneFiles) core_createFauxFragDoneFiles()
#     q(save = 'no', status = 1, runLast = FALSE)
#   }
# }
