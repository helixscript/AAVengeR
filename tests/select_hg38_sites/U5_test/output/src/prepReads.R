...
if(! file.exists(file.path(opt$outputDir, opt$prepReads_readsTable))){
  updateLog('Error - the input data file does not exist.')
...
if(nrow(reads) == 0){
  updateLog('Error - the input table contained no rows.')
  if(opt$core_createFauxFragDoneFiles) core_createFauxFragDoneFiles()
  q(save = 'no', status = 1, runLast = FALSE) 
}
...
if(nrow(reads) == 0){
  updateLog('Error - no reads remain after filtering for reads aligning to the vector.')
  if(opt$core_createFauxFragDoneFiles) core_createFauxFragDoneFiles()
  q(save = 'no', status = 1, runLast = FALSE) 
}
...
if(! 'leaderSeqHMM' %in% names(reads)){
    ...
    if(nrow(vectorHits2) == 0){
      updateLog('Error - no reads aligned to the vector for leader seq generation.')
      if(opt$core_createFauxFragDoneFiles) core_createFauxFragDoneFiles()
      q(save = 'no', status = 1, runLast = FALSE) 
    }
