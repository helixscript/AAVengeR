...
x <- lapply(split(sites, sites$sample), function(x){
        ...
        d <- tibble(remnantLen = as.integer(sub('\\.\\.', '', stringr::str_extract(x$repLeaderSeqMap, '\\.\\.\\d+'))) + opt$buildAAVremnantPlots_ITRseqStart,
                    bin = cut(remnantLen, breaks = c(-Inf, range, Inf), labels = FALSE),
       ...
       p <- ggplot(d, aes(bin, n, fill = r2)) + 
             ...
             scale_x_continuous(breaks = range,
                               labels = range2, 
                               limits = c(opt$buildAAVremnantPlots_NTbinSize, NA)) +
             ...
             geom_vline(xintercept = cut(opt$buildAAVremnantPlots_ITRlength + opt$buildAAVremnantPlots_NTbinSize, breaks = c(-Inf, range, Inf), labels = FALSE)) +
             ggtitle(paste0(x$subject[1], ' | ', x$sample[1])) + 
   ...
   ggsave(file.path(opt$outputDir, opt$buildAAVremnantPlots_outputDir, paste0(x$trial[1], '~', x$subject[1], '~', x$sample[1], '.png')), p, width = opt$buildAAVremnantPlots_plotOutputWidthInches, units = 'in')
