...
frags <- bind_rows(lapply(split(frags, paste(frags$trial, frags$subject)), function(x){
  ...
  message('frags start: ', nrow(x))
  x <- left_join(x, select(distinct(d), leaderSeq.anchorReads, leaderSeqGroup), by = 'leaderSeq.anchorReads')
  message('frags end: ', nrow(x))
  x
}))
...
# (!) This should be patient specific.
...
# select(o, randomLinkerSeq.adriftReads, uniqueSample, posid, fragWidth, reads, repLeaderSeq) 
