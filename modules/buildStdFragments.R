...
# UMIs are corrected as well if the processAdriftReadLinkerUMIs parameter is set to TRUE.

for (p in c('lubridate', 'dplyr', 'parallel', 'data.table', 'GenomicRanges', 'Biostrings', 'igraph', 'dtplyr', 'RMariaDB')) suppressPackageStartupMessages(library(p, character.only = TRUE))
...
if(opt$buildStdFragments_standardizeIntegrationPositions){
  ...
  g2 <- unlist(GRangesList(parLapply(cluster, split(g, g$s), function(x){
          ...
          source(file.path(opt$softwareDir, 'lib', 'gintools.R'))
...
if(nrow(frags_multPosIDs) > 0){
  ...
  frags_multPosIDs$posid2  <- sub('\\.\\d+$', '', frags_multPosIDs$posid)
  ...
  if(any(frags_multPosIDs$returnToFrags == TRUE) & opt$buildStdFragments_salvageMultiHitReads){
    ...
    m <- bind_rows(lapply(split(m, m$readID), function(x){
      if(n_distinct(x$fragID) > 1 & n_distinct(x$posid2) == 1){
        if(x$strand[1] == '+'){
    ...
    frags_multPosIDs <- subset(frags_multPosIDs, ! readID %in% m$readID) %>% dplyr::select(-s, -posid2, -returnToFrags)
...
if(opt$buildStdFragments_evalFragAnchorReadSeqs){
  ...
  # a clear separation between the first and second position id. If a clear distinction can not be found 
...
if(nrow(frags_multPosIDs) > 0 & opt$buildStdFragments_createMultiHitClusters){
  ...
  if(nrow(multiHitNet_replicates) > 0){
    ...
    cluster <- makeCluster(opt$buildStdFragments_CPUs)
    clusterSetRNGStream(cluster, 1)
    clusterExport(cluster, c('opt', 'multiHitFragData'))
  ...
  if(opt$buildStdFragments_calcMultiHitClusterAbunds | opt$buildStdFragments_calcMultiHitClusterNodeAbunds){
    ...
    multiHitClusters$clusterSonicLengths <- NA
    multiHitClusters$nodeSonicLengths <- list(data.table())
  }
