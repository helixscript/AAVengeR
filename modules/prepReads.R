...
if(nrow(vectorHits) > 0){
  updateLog(paste0(ppNum(n_distinct(vectorHits)), ' reads aligned to the vector and will be excluded.'))
...
updateLog(paste0('Leader sequences determined for ', sprintf("%.2f%%", (n_distinct(m$id)/n_distinct(reads$readID))*100), ' of unique read pairs.'))
...
updateLog('Removing identified leader sequences from anchor reads.')
...
updateLog('Trimming adrift read over-reading.')
