...
# John K. Everett, Ph.D.
# 
# This script accepts prepared reads from the prepReads module and aligns
...
alignReads <- function(r, refGenome, minPercentSeqID, maxQstart, dir){
  ...
  if(length(f) > 0){
    b <- rbindlist(lapply(f, function(x){
           ...
           dplyr::filter(b, queryPercentID >= minPercentSeqID, 
                            tNumInsert  <= opt$alignReads_genomeAlignment_blatMaxtNumInsert, 
                            qNumInsert  <= opt$alignReads_genomeAlignment_blatMaxqNumInsert, 
                            tBaseInsert <= opt$alignReads_genomeAlignment_blatMaxtBaseInsert, 
                            qBaseInsert <= opt$alignReads_genomeAlignment_blatMaxqBaseInsert, 
                            qStart <= maxQstart) %>%
           dplyr::select(qName, strand, qSize, qStart, qEnd, tName, tStart, tEnd, queryPercentID)
        }))
  }
...
anchorReadAlignments <- left_join(anchorReadAlignments, select(reads, readID, anchorReadSeq, adriftReadSeq, leaderSeq), by = 'readID')
